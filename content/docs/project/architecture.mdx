...
├── content/docs/               # PRD content (MDX), served by Fumadocs at /docs (src/app/docs)
├── .github/workflows/          # production, preview, playwright (disabled)
...
erDiagram
    ...
    player {
        ...
        boolean is_captain
        integer jersey_number "check: 0..99; unique per team in the action layer"
    ...
    attendance {
        ...
        text player_id FK
        uuid session_id FK "nullable; unique (player_id, session_id) when set"
        attendance_status status
        date date "FK with session_id; unique (player_id, date) when session-less"
        varchar reason
    }
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------ |
| `user.team_id`, `rule.team_id`, `asset.team_id`, `test_type.team_id`, `training_session.team_id`, `league_team.*`, `league_team_roster.*` | `team` / `league` / `player`                       | **cascade**                                                  |
| `session.user_id`, `account.user_id`, `player.id`, `coach.id`                                                                                                   | `user`                                             | **cascade**                                                  |
| `attendance.player_id`, `attendance.(session_id, date)`, `match_player_stats.player_id`, `test_result.player_id`, `match.league_id`, `match_player_stats.match_id`                               | `player` / `league` / `match`                      | **cascade**                                                  |
| `asset.assigned_to`, `match.location_id`, `training_session.coach_id`, `training_session.location_id`                                  | `user` / `training_session` / `location` / `coach` | **set null**                                                 |
...
`src/drizzle/migrations/` (dialect postgresql, 11 migrations): `0000_base_tables`, `0001_update_asset_table` (adds `OBSOLETE`, `assigned_to`, `acquired_date`), `0002_complete_doctor_doom` (recreates `test_result.type_id` FK as `ON DELETE restrict`), `0003_update_team_image` (renames `team.logo_url` → `team.image`, to `text`), `0004_add_achievement_table`, `0005_update_test_type_unit_enum`, `0006_lowercase_all_enum_values`, `0007_lowercase_achievement_type`, `0008_drop_global_jersey_unique` (jersey uniqueness moves to the action layer), `0009_attendance_per_session` (composite `(session_id, date)` FK + the two partial unique indexes), `0010_unique_asset_name` (drops the global `asset.name` unique, adds `team_asset_name` on `(team_id, name)`, drops the redundant `attendance.team_id`). Raw bootstrap/seed SQL lives in `src/drizzle/sql/` (`team.sql` default team, `admin_user.sql`, `team_rule.sql`, `periodic-testing.sql`, and `seed_data.sql`, a re-runnable mock-data importer targeting 20 teams / 100 users / 100 rows per domain).
...
sequenceDiagram
    ...
    Note over U: authClient.useSession() keeps the session fresh on the client#59;<br/>AppShell also redirects to /login if the client resolves unauthenticated
`}
/>
...
- **Database errors**: `src/db/pg-error.ts` maps Postgres SQLSTATE codes (`PgErrorCode`) to user-friendly messages via `getDbErrorMessage()`, unwrapping `DrizzleQueryError` → `pg.DatabaseError`. Actions catch specific constraint names (e.g. `unique_player_per_date` / `unique_player_per_session`, `attendance_player_id_player_id_fk`, the `test_result → test_type` FK) to return friendly errors. Jersey-number uniqueness has no database constraint: `isJerseyNumberTaken` pre-checks it in the action layer instead, since `player` only reaches its team through `user`.
...
> The docs are no longer a separate deployment: Fumadocs pages live in `src/app/docs` (content in `content/docs/`) and ship with the main app build.
...
- **Local setup**: see [README.md](https://github.com/diosvo/team-management/blob/main/README.md) and [DEVELOPMENT.md](https://github.com/diosvo/team-management/blob/main/DEVELOPMENT.md). Package manager is **pnpm**; run `pnpm dev` (Next dev server), `pnpm db:migrate` / `pnpm db:studio` for the database. Env vars go in `.env` (validated by `env.config.ts`).
...
Sourced from [TODO.md](https://github.com/diosvo/team-management/blob/main/TODO.md), a schema-review checklist. Highest-priority (⭐) items reject real-world data or block real workflows:

- ⭐ **Add a `match` status enum**: scores default to 0, so a scheduled-but-unplayed match is indistinguishable from a 0–0 result.
- ⭐ **`match.home_team` / `away_team` have no `onDelete`**: deleting a team with match history throws a raw FK error.
...
- Recently closed: jersey-number uniqueness per team (`0008`), attendance per session (`0009`), `asset.name` unique per team and the redundant `attendance.team_id` dropped (`0010`), `test_result.date` NOT NULL (`0011`), the `league_team_roster` → `league_team` composite FK and the derivable `league.status` column dropped (`0012`).
...
- **Simplification candidates**: `rule` could fold into `team`.
...
| **Author / maintainer** | Dios Vo, [vtmn1212@gmail.com](mailto:vtmn1212@gmail.com) ([LinkedIn](https://www.linkedin.com/in/diosvo/)) |
...
| **PRD** | Product Requirements Document: specs under `content/docs/`, served by the app at `/docs` |
