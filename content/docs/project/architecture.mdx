...
- **Make `league_team_roster` reference `league_team`**; **make `test_result.date` NOT NULL**.
- Recently closed: jersey-number uniqueness per team (`0008`), attendance per session (`0009`), `asset.name` unique per team and the redundant `attendance.team_id` dropped (`0010`).
...
- **Simplification candidates**: `league.status` is derivable, `rule` could fold into `team`.
