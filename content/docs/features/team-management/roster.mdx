...
- **FR-5:** Jersey numbers are unique per team, enforced in the action layer (`isJerseyNumberTaken`, `src/db/player.ts`) because `player` reaches its team through `user`. A duplicate is rejected with “Jersey number '`<n>`' is already taken”.
...
- `jersey_number`: integer (player only; 0–99, unique per team — checked in the action layer, not by a database constraint)
