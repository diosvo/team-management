...
There is no `result` column and no match status column: the win/draw/loss badge is computed from the two scores. Because scores default to 0, a scheduled-but-unplayed match currently looks like a 0–0 draw (tracked in [TODO.md](https://github.com/diosvo/team-management/blob/main/TODO.md)).
