...
- **FR-6:** Duplicate records for the same player on the same date are prevented by the `unique_player_per_date` index. This also blocks a second record on a two-session day, which is an open item in [TODO.md](https://github.com/diosvo/team-management/blob/main/TODO.md).
