...
<Mermaid
  chart={`
flowchart LR
    A["Manual click:<br/>Download report"] --> B["POST /api/reports/dashboard"]
    B --> C["puppeteer-core + @sparticuz/chromium:<br/>load live dashboard URL,<br/>session cookies forwarded"]
    C --> D["Strip DOM to<br/>analytics grid"]
    D --> E["page.pdf → Buffer"]
