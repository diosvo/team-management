...
- Fields: date, start time, end time, location. Coach is auto-assigned (must be Head Coach).
...
### Data model (`src/drizzle/schema/training.ts`)

TrainingSession (`training_session`):

- `session_id`: uuid PK
- `team_id`: FK → team (cascade on team delete)
- `coach_id`: FK → coach, nullable (set null on coach delete)
- `location_id`: FK → location, nullable (set null on location delete)
- `date`: date
- `start_time` / `end_time`: time, both required
- `status`: enum `session_status` (`scheduled` | `active` | `completed` | `cancelled`), default `scheduled`
