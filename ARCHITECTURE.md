...
- **PDF generation**: two paths exist. Server-side dashboard export runs via Puppeteer (`src/lib/puppeteer.ts` + reports route), and client-side registration forms via `pdf-lib` + `@pdf-lib/fontkit` (`registration/_helpers/pdf.ts`, with AcroForm field auto-mapping and CSV export).
