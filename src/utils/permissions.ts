...
type RoleConfig = Partial<Record<Resource, Array<Action>>>;
...
export function can(
  ...
  if (ROLE_CONFIG[role]?.[resource]?.includes(action)) return true;
 ...
 *   ability.canAny(['roster:edit', 'matches:create']); // true
 * ```
 */
export function defineAbility(role: UserRole, isCaptain?: boolean): Ability {
  ...
  return {
    can: _can,
    canAll: (actions: Array<Permission>) => actions.every(_can),
    canAny: (actions: Array<Permission>) => actions.some(_can),
  };
}
